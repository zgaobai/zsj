    ...
    <div class="main">
        <div class="header">
            ...
            <div class="right">
                <div class="info">
                    {{info.name |name}}
                   <span class="sign">社保卡</span>
                </div>
                <p class="address">社保局</p> 
            </div>
            <p class="num">就诊卡号：<span>{{info.cardNumber}}</span></p>
        </div>
        ...
        <div class="content">
            <div class="pay-box">
                <div class="pay" v-for="(item, index) in showImg" :key="index" @click="active(index)">
                    ...
                    <div class="text">{{item.name}}<span class="tips">{{item.text}}</span></div>
                    <div class="radio">
...
export default {
    ...
    methods: {
        active (index) {
            switch(index){
                case 0 :
                this.$router.push('/Personal/Center/Info/Vcard');
                break;
                case 1 :
...
.main
    ...
    padding-top 20px
    .header
        background-image: url('../../../static/imgs/bg_green.png')
